...
extension EventsByDayTableViewController: UITableViewDelegate, UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
    let title = "Delete"
    let deleteAction = UIContextualAction(style: .destructive, title: title) { (action, view, completionHandler) in
      let dismiss = self.dailyHygieneEvents.count == 1 ? true : false
      self.eventsToBeDeletedDelegate?.notifyCVEventsToBeDeleted(dayOfEvent: self.dayOfEvent, indexOfEvent: indexPath.row, dismissVC: dismiss)
      self.dailyHygieneEvents.remove(at: indexPath.row)
...
//
//        print("In handelDeleteUser", dayOfEvent, indexPath.row, hygieneCalendarViewModel.currentMonth, dailyHygieneEvents.count)
//
