...
enum UserCalendarViewModelItemType {
  case addUser
  case filler
...
class UserCalendarCell: BaseUserTableViewCell<CalendarUsers> {
...
class UserCalendarTableViewController: GenericTableViewController<UserCalendarCell, CalendarUsers> {
  ...
  @objc func notificationUpdateTableView(_ notificaiton: Notification) {
    ...
    if let newUser = userInfo["newUser"] as? CalendarUsers {
      print(UserDefaults.standard.bool(forKey: kEditUser) ?? "no value")
      if (UserDefaults.standard.bool(forKey: kEditUser)) {
        for user in calendarUsers {
          if user.userID == newUser.userID {
            user.name = newUser.name
            user.image = newUser.image
            UserDefaults.standard.set(false, forKey: kEditUser)
            tableView.reloadData()
            }
          }
      } else {
...
//        }
  ...
  @objc private func deleteUserCalendar(user: CalendarUsers, index: IndexPath) {
    ...
    UserDefaults.standard.set(nextCurrentUser.userID.uuidString, forKey: kCurrentCalendarUserID)
